...
def test_alembic_upgrade_head_creates_all_tables(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [str(repo / ".venv/bin/alembic"), "upgrade", "head"],
...
def test_alembic_downgrade_to_base(tmp_path: Path) -> None:
    ...
    up = subprocess.run([str(repo / ".venv/bin/alembic"), "upgrade", "head"],
                        cwd=repo, env=env, capture_output=True, text=True)
    assert up.returncode == 0, up.stderr

    down = subprocess.run([str(repo / ".venv/bin/alembic"), "downgrade", "base"],
                          cwd=repo, env=env, capture_output=True, text=True)
