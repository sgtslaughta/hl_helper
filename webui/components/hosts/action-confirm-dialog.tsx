    ...
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg rounded border border-hairline bg-surface p-5">
        ...
        <ul className="mb-3 list-disc space-y-1 pl-5 text-sm text-text">
          {entry.risks.map(r => <li key={r}>{r}</li>)}
        </ul>

        <div className="space-y-2">
          <Disclosure label="Technical details" storageKey={`action-${action}-tech`}>
            <div className="space-y-1 font-mono text-xs">
              <div><span className="text-text-dim">Method:</span> {entry.technical.method}</div>
              <div><span className="text-text-dim">Path:</span> {path}</div>
              <div><span className="text-text-dim">Payload preview:</span></div>
              <pre className="overflow-auto rounded bg-surface-2 p-2">{JSON.stringify(entry.technical.samplePayload, null, 2)}</pre>
              <div><span className="text-text-dim">Idempotency-Key:</span> generated per request (UUID v4)</div>
            </div>
          </Disclosure>
        ...
        <footer className="mt-4 flex justify-end gap-2">
          <button type="button" onClick={onClose} className="rounded border border-hairline px-3 py-1.5 text-sm text-text hover:bg-surface-2">
            Cancel
          </button>
          <button
            type="button"
