...
import { listHosts, type Host } from '@/lib/api/hosts';
import { listPendingTokens, type PendingToken } from '@/lib/api/enrollment';
...
export function HostListTable({ status, search, onEnroll }: Props) {
  ...
  if (hostsQ.isError) return <EmptyState title="Failed to load hosts" description="Please try again" />;
  ...
  return (
    <div className="rounded border border-hairline bg-surface overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {pending.map(t => <PendingHostRow key={t.id} token={t} />)}
          {hosts.map(h => <ActiveHostRow key={h.id} host={h} />)}
        </tbody>
      </table>
    </div>
  );
