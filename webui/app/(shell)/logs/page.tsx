...
import { type FacetBucket, type LogRow, listLogs } from '@/lib/api/logs';
...
export default function LogsHubPage() {
	...
	const successRate =
		rows.length > 0 ? Math.round(((rows.length - failureCount) / rows.length) * 100) : 100;
	const uniqueHosts = new Set(rows.map(r => r.host_id)).size;

	// Build histogram bins from rows. Window is derived from the actual data
	// span (min..max ts) so the sparkline shows real distribution; falls back
	// to last 1h when there are no rows. Previously used `cursor` (a base64
	// pagination cursor) as a date which produced NaN bucket times.
	const tsList = rows.map(r => new Date(r.ts).getTime()).filter(t => Number.isFinite(t));
	const tMax = tsList.length > 0 ? Math.max(...tsList) : Date.now();
	const tMin = tsList.length > 0 ? Math.min(...tsList) : tMax - 60 * 60 * 1000;
	const span = Math.max(tMax - tMin, 60 * 1000); // at least 1min so bucket size > 0
	const bucketSize = span / 60;
	const bins = Array.from({ length: 60 }).map((_, i) => {
		const start = tMin + i * bucketSize;
		...
		<div className="flex h-full flex-col bg-surface">
			...
			<div className="border-b border-hairline px-6 py-4">
				...
				<p className="text-sm text-text-dim mt-1">
					Search and analyze activity logs across all hosts
				</p>

				{/* Hero metrics */}
				<div
					className={`grid gap-3 mt-4 ${density === 'lean' ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-2 sm:grid-cols-4'}`}
				>
					{/* Total Events */}
					...
					<div className="rounded border border-hairline bg-surface-2 px-3 py-2">
						<div className="flex items-baseline justify-between gap-2">
							<span className="text-xs text-ok font-semibold uppercase tracking-wider">
								Success
							</span>
			...
			<div className="flex flex-1 overflow-hidden">
				...
				<div className="flex-1 flex flex-col gap-4 overflow-hidden p-4">
					...
					{!loading && rows.length === 0 && (
						<div className="text-sm text-text-dim p-6">No logs found</div>
					)}
					{!loading && rows.length > 0 && (
