...
describe('MissionControlShell', () => {
  ...
  it('focus tab key 2 switches focus pane to Posture', async () => {
    render(wrap(<MissionControlShell initialHostId="h1" />));
    await screen.findByRole('tab', { name: /Overview/ });
    fireEvent.keyDown(window, { key: '2' });
    expect(await screen.findByRole('tab', { name: /Posture/ })).toHaveAttribute('aria-selected', 'true');
  });

  it('action key w switches action pane to Terminal', async () => {
    render(wrap(<MissionControlShell initialHostId="h1" />));
