...
export const riskCatalog: Record<ActionType, RiskEntry> = {
  reboot: {
    ...
    rollback: 'Reboot is not reversible from this UI. If the host fails to return, use console access on the affected machine.',
    technical: {
      method: 'POST',
  ...
  'shell-exec': {
    ...
    rollback: 'Effects depend on the command. Review command output and audit log; no automatic rollback.',
    technical: {
      method: 'POST',
  ...
  'pkg-update': {
    ...
    rollback: 'Most package managers support pinning the previous version; rollback is host-specific and not automated here.',
    technical: {
      method: 'POST',
  ...
  'revoke-host': {
    ...
    rollback: 'Re-enroll the host with a fresh enrollment token. The previous certificate cannot be unrevoked.',
    technical: {
      method: 'POST',
  ...
  'delete-host': {
    ...
    rollback: 'No rollback. Re-enroll to recreate, but the new host has a new id and history starts empty.',
    technical: {
      method: 'DELETE',
  ...
  'mint-enrollment-token': {
    ...
    rollback: 'Revoke the token before redemption. After redemption the resulting host can be revoked separately.',
    technical: {
      method: 'POST',
